{Portable Network Graphics Delphi 1.42 mc 2b  (19 April 2002) }
...
{full partial transparent support, speed improvements, saving }
{using ADAM 7 interlacing, better error handling, the best    }
{compression for the final image ever. And now it's truly     }
{able to read about any png image.                            }
...
{$DEFINE ObjectPascal}           //Disable fat vcl units (perfect to small apps)
...
uses
 Windows {$IFDEF ObjectPascal}, SysUtils, Classes, Graphics{$ENDIF}
 {$IFDEF Debug}, dialogs{$IFNDEF ObjectPascal}, SysUtils{$ENDIF}{$ENDIF},
 MMPropZLIB;
...
type
  ...
  private
    {Used with property ItemFromClass}
    function GetClassItem(ChunkClass: TChunkClass): TChunk;
    {Used with property Item}
    function GetItem(Index: Cardinal): TChunk;
  public
    {Add a new chunk using the class from the parameter}
    function Add(ChunkClass: TChunkClass): TChunk; reintroduce;
    {Returns pointer to the first chunk of class}
    property ItemFromClass[ChunkClass: TChunkClass]: TChunk read GetClassItem;
  ...
  {$IFNDEF ObjectPascal}
    ...
    {avaliable (ObjectPascal trigger not set)}
  ...
  type
    ...
    public
      ...
      function Read(var Buffer; Count: Longint): Integer; virtual; abstract;
  ...
  TPngObject = class{$IFDEF ObjectPascal}(TGraphic){$ENDIF}
  protected
    {Gamma table values}
  ...
  private
    ...
    function GetAlphaScanline(const Index: Integer): Pointer;
    function GetScanline(const Index: Integer): Pointer;
  protected
    {Returns/sets image width and height}
    function GetWidth: Integer; {$IFDEF ObjectPascal}override;{$ENDIF}
    function GetHeight: Integer; {$IFDEF ObjectPascal}override; {$ENDIF}
    procedure SetWidth(Value: Integer);  {$IFDEF ObjectPascal}override; {$ENDIF}
    procedure SetHeight(Value: Integer);  {$IFDEF ObjectPascal}override;{$ENDIF}
    {Assigns from another TPNGObject}
    procedure AssignPNG(Source: TPNGObject);
    {Returns if the image is empty}
    function GetEmpty: Boolean; {$IFDEF ObjectPascal}override; {$ENDIF}
    {Returns if the image uses partial transparency}
    function HasPartialTransparency: Boolean;
  ...
  public
    ...
    procedure SaveToClipboardFormat(var AFormat: Word; var AData: THandle;
      var APalette: HPalette); {$IFDEF ObjectPascal}override;{$ENDIF}
    procedure LoadFromClipboardFormat(AFormat: Word; AData: THandle;
      APalette: HPALETTE);  {$IFDEF ObjectPascal}override;{$ENDIF}
    {Calling errors}
    ...
    property AlphaScanline[const Index: Integer]: Pointer read GetAlphaScanline;
    {Returns pointer to the header}
    property Header: TChunkIHDR read GetHeader;
    {Returns if the image uses partial transparency}
    property PartialTransparency: Boolean read HasPartialTransparency;
    {Assigns from another object}
    procedure Assign(Source: TPersistent);{$IFDEF ObjectPascal}override;{$ENDIF}
    {Assigns to another object}
    procedure AssignTo(Dest: TPersistent);{$IFDEF ObjectPascal}override;{$ENDIF}
    {Assigns from a windows bitmap handle}
    procedure AssignHandle(Handle: HBitmap);
    {Draws the image into a canvas}
    procedure Draw(ACanvas: TCanvas; const Rect: TRect);
      {$IFDEF ObjectPascal}override;{$ENDIF}
    ...
    constructor Create; {$IFDEF ObjectPascal}override;{$ENDIF}
    destructor Destroy; override;
    {$IFNDEF ObjectPascal}procedure LoadFromFile(const Filename: String);{$ENDIF}
    {$IFNDEF ObjectPascal}procedure SaveToFile(const Filename: String);{$ENDIF}
    procedure LoadFromStream(Stream: TStream); {$IFDEF ObjectPascal}override;{$ENDIF}
    procedure SaveToStream(Stream: TStream); {$IFDEF ObjectPascal}override;{$ENDIF}
  end;
...
//    property Name: TChunkName read fName; Not BCB Compatible
  ...
  TChunktRNS = class(TChunk)
    ...
    MaskDC: HDC;
  protected
    {Returns if the transparency for the palette has partial}
    {transparency information}
    function HasPalettePartialTransparency: Boolean;
    {Build/clear mask for the image}
    procedure BuildMask;
    procedure ClearMask;
  public
...
begin
  ...
  begin
    {Returns value}
    Longint(Result) := pLongint(Longint(fMemory) + Longint(Index *
      SizeOf(Pointer)))^;
  end
  else
...
begin
  ...
  begin
    ...
    if Position < Count then
      System.Move(Pointer(Longint(fMemory) + Longint(Position) * SizeOf(Pointer))^,
        Pointer(Longint(fMemory) + (Longint(Position) + 1) * SizeOf(Pointer))^,
        (Count - Position) * SizeOf(Pointer));
    {Sets item}
    Item[Position] := Value;
  end;
end;
...
begin
  if (Index <= Count - 1) then
    {Returns value}
    pLongint(Longint(fMemory) + Longint(Index * SizeOf(Pointer)))^ :=
      Longint(Value);
end;
...
begin
  ...
  if (ChunkClass = TChunkIHDR) or (ChunkClass = TChunkIDAT) or
    ...
    fOwner.LoadError(EPngError, EPNGCannotAddChunkText)
  ...
  {There must have an IEND and IHDR chunk}
    ...
    fOwner.LoadError(EPngError, EPNGCannotAddInvalidImageText)
  else
  begin
    {Get common chunks}
    ...
    {Add to the list}
    if (ChunkClass = TChunkgAMA) or (ChunkClass = TChunktRNS) then
      Insert(Result, IHDR.Index + 1)
    else {All other chunks}
      Insert(Result, IEND.Index - 1);
  end {if}
end;
...
function TPNGList.GetClassItem(ChunkClass: TChunkClass): TChunk;
var
  i: Integer;
begin
...
function TStream.GetSize: Longint;
  ...
  begin
    if not WriteFile(fHandle, Buffer, Count, LongWord(Result), nil) then
      Result := -1;
  end;

  {Reads data from the file}
  function TFileStream.Read(var Buffer; Count: Longint): Integer;
  begin
...
begin
  ...
  {$IFDEF CheckCRC}
    ...
    begin
      ...
      Owner.LoadError(EPngInvalidCRC, EPngInvalidCRCText);
      exit;
    end
...
begin
  ...
  {Copy data}
  CopyMemory(Data, @fKeyword[1], Length(Keyword) + 1);
...
begin
  ...
  begin
    ...
    Owner.LoadError(EPNGError, EPNGCannotAssignChunkText);
end;
...
begin
  ...
  begin
    ...
    Result := False;
    Owner.LoadError(EPNGInvalidIHDR, EPNGInvalidIHDRText);
    exit;
  end;
  ...
  begin
    Result := False;
    Owner.LoadError(EPNGSizeExceeds, EPNGSizeExceedsText);
    exit;
  ...
  begin
    Result := False;
    Owner.LoadError(EPNGUnknownCompression, EPNGUnknownCompressionText);
    exit;
  end;
  ...
  begin
    Result := False;
    Owner.LoadError(EPNGUnknownInterlace, EPNGUnknownInterlaceText);
    exit;
  end;
...
{Returns if the transparency for the palette has partial}
{transparency information}
function TChunktRNS.HasPalettePartialTransparency: Boolean;
var
  i: Integer;
  Diff: Integer;
begin
  Result := FALSE; {Initial result}
  Diff := 0; {Initialize to count}
  {The image only has partial transparency if:}
  {* More than one item has transparency set to 0}
  {* Any item has transparency different from 0 and 255}
  FOR i := 0 TO 255 DO
    if (PaletteValues[i] <> 255) then
      if (PaletteValues[i] <> 0) then Result := True else inc(Diff);
  {More than one item has transparency 0}
  if Diff > 1 then Result := TRUE;
end;

{Clears old mask}
procedure TChunktRNS.ClearMask;
begin
  {Delete objects}
...
{Build a mask to the image}
procedure TChunktRNS.BuildMask;
begin
  {Creates the mask bitmap}
  Mask := CreateBitmap(Header.Width, Header.Height, 1, 1, nil);
  MaskDC := CreateCompatibleDC(0);
  SelectObject(MaskDC, Mask);
end;

{Transparency information chunk being created}
constructor TChunktRNS.Create(Owner: TPngObject);
begin
  {Initialize}
  Mask := 0; MaskDC := 0;
  {Let inherited create it}
  inherited Create(Owner);
end;

{Transparency information chunk being destroyed}
destructor TChunktRNS.Destroy;
begin
  {Free mask if being used}
  ClearMask();
  {Let inherited destroy it}
  inherited Destroy;
end;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        begin
          Owner.LoadError(EPNGMissingMultipleIDAT, EPNGMissingMultipleIDATText);
          result := -1;
          exit;
        end;
...
begin
  ...
  if Index > Count - 1 then
    Owner.LoadError(EPNGError, EPNGUnknownPalEntryText)
  else
    {Returns the item}
...
begin
  ...
  begin
    {Raise error}
    Result := FALSE;
    Owner.LoadError(EPNGInvalidPalette, EPNGInvalidPaletteText);
    exit;
  end {if Size mod 3 <> 0};
...
begin
  ...
  {$IFDEF ObjectPascal}else if Source is TBitmap then
    AssignHandle(TBitmap(Source).Handle){$ENDIF}
...
begin
  {Initialize gamma}
  InitializeGamma();
  {Free all the objects and memory}
...
{Raises an error}
procedure TPngObject.LoadError(ExceptionClass: ExceptClass; Text: String);
begin
...
{$IFNDEF ObjectPascal}
  ...
  begin
    ...
    begin
      ...
      LoadError(EPNGNotExists, EPNGNotExistsText);
      exit;
    end;
...
begin
  ...
  BitmapInfoHeader.biHeight := -Header.Height;
  ...
  case Header.BitmapInfo.bmiHeader.biBitCount of
    ...
    8: if Header.ColorType = COLOR_GRAYSCALEALPHA then
      FOR j := 1 TO Header.Height DO
      begin
        {Process all the pixels in this line}
        FOR i := 0 TO Header.Width - 1 DO
...
begin
  ...
  if Chunks.Count = 0 then Exit;
  Header := Chunks.GetItem(0) as TChunkIHDR;

  {Copy the data to the canvas}
  {$IFDEF PartialTransparentDraw}
  if Self.HasPartialTransparency then
    DrawPartialTrans(ACanvas{$IFDEF ObjectPascal}.Handle{$ENDIF}, Rect)
  else
  {$ENDIF}
    StretchDiBits(ACanvas{$IFDEF ObjectPascal}.Handle{$ENDIF}, Rect.Left,
...
begin
  ...
  begin
    LoadError(EPNGInvalidFileHeader, EPNGInvalidFileHeaderText);
    Exit;
  end;
  ...
  repeat
    ...
    begin
      ...
      LoadError(EPNGUnexpectedEnd, EPNGUnexpectedEndText);
    end;
    ...
    begin
      Chunks.Count := ChunkCount - 1;
      LoadError(EPNGIHDRNotFirst, EPNGIHDRNotFirstText);
      exit;
    end;
    ...
    {$IFDEF ErrorOnUnknownCritical}
      ...
      begin
        ...
        LoadError(EPNGUnknownCriticalChunk, EPNGUnknownCriticalChunkText);
      end;
    {$ENDIF}
  ...
  if not HasIDAT then
    LoadError(EPNGNoImageData, EPNGNoImageDataText);
end;

{Changing height is not supported}
procedure TPngObject.SetHeight(Value: Integer);
begin
  LoadError(EPNGError, EPNGCannotChangeSizeText);
end;
...
begin
  {Clipboard compatible with bitmaps}
  AFormat := CF_BITMAP;
end;
...
begin
  ...
  FOR j := 0 TO Chunks.Count - 1 DO
    TChunk(Chunks.Item[j]).SaveToStream(Stream);
end;
...
begin
  ...
  else if (Dest is TBitmap) and (Header <> nil) then
    TBitmap(Dest).Handle := Header.ImageHandle
  else
...
procedure TPngObject.AssignHandle(Handle: HBitmap);
const
  NumberOfChunks: Array[Boolean] of Integer = (3, 4);
var
...
begin
  ...
  ClearChunks();
  Chunks.Count := NumberOfChunks[HasPalette];  {Header, (PLTE), IDAT, IEND}
  ...
  {Adjust pointers to the chunks}
  Chunks.SetItem(0, Header);
  if HasPalette then Chunks.SetItem(1, PLTE) else Chunks.SetItem(1, IDAT);
  if HasPalette then Chunks.SetItem(2, IDAT) else Chunks.SetItem(2, IEND);
  if HasPalette then Chunks.SetItem(3, IEND);
...
{Returns if the image uses partial transparency}
function TPngObject.HasPartialTransparency: Boolean;
//var
//  TransparencyChunk: TChunk;
begin
  case Header.ColorType of
    {This modes always have partial transparency}
    COLOR_RGBALPHA, COLOR_GRAYSCALEALPHA: Result := TRUE;
    {RGB and partial without alpha}
    COLOR_RGB, COLOR_GRAYSCALE: Result := FALSE;
    {Palettes may have full or partial transparent modes}
    COLOR_PALETTE:
    begin
      {Finds tRNS chunk}  (*
      TransparencyChunk := Chunks.ItemFromClass[TChunktRNS];
      if (TransparencyChunk <> nil) then
        Result := TChunktRNS(TransparencyChunk).HasPalettePartialTransparency
      else
        {No transparency information was found}
        Result := FALSE;*)
      Result := FALSE;
    end {COLOR_PALETTE};
    {Unknown color modes}
...
function TPngObject.GetAlphaScanline(const Index: Integer): Pointer;
begin
  with Header do
    if (ColorType = COLOR_RGBALPHA) or (ColorType = COLOR_GRAYSCALEALPHA) then
      Longint(Result) := Longint(ImageAlpha) + (Index * Longint(Width))
    else Result := nil;  {In case the image does not use alpha information}
end;

{Returns a png data scanline}
function TPngObject.GetScanline(const Index: Integer): Pointer;
begin
  with Header do
    Longint(Result) := Longint(ImageData) + (Index * BytesPerRow);
end;
...
procedure TPngObject.LoadFromClipboardFormat(AFormat: Word; AData: THandle;
  APalette: HPALETTE);
begin
end;
...
initialization
  ...
  {$IFDEF ObjectPascal}{$IFDEF RegisterGraphic}
