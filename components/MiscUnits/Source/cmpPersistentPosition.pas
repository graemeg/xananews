...
var
  ...
  reg: TExSettings;
begin
  if fMoved or not fEnabled then
    Exit;
  fMoved := True;
...
{ *----------------------------------------------------------------------*
  ...
  *----------------------------------------------------------------------* }
procedure TPersistentPosition.SetValue(const valueName: string; value: Integer);
var
  reg: TExSettings;
begin
...
begin
  if CreateReg(False, reg) then
    try
      try
        Result := Rect(reg.IntegerValue['Left'], reg.IntegerValue['Top'], reg.IntegerValue['Width'],
          reg.IntegerValue['Height']);
