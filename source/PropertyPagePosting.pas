...
uses
  ...
  Dialogs, PropertyPageDefaultsForm, PropertyPageForm, StdCtrls, ExtCtrls, unitSettings;
...
procedure TfmPropertyPagePosting.InitializeControls(
  settings: TPostingSettings);
var
  idx : Integer;
begin
  ...
  case settings.TextPartStyle of
    tpNNTP : rbTextPartNNTP.Checked := True;
    tpMIME : rbTextPartMIME.Checked := True;
    tpQuotedPrintable : rbTextPartQuotedPrintable.Checked := True;
    tpFlowed : rbTextPartFlowed.Checked := True
  end;
...
procedure TfmPropertyPagePosting.PopulateControls(
...
begin
  ...
  begin
    for i := 0 to TSpellChecker.LanguageCount - 1 do
      cbDefaultISpellLanguage.Items.Add(TSpellChecker.Language (i).Name);

    if (settings.DefaultSpellLanguage >= 0) and (settings.DefaultSpellLanguage < TSpellChecker.LanguageCount) then
      idx := settings.DefaultSpellLanguage
    else
      if TSpellChecker.LanguageCount > 0 then
        idx := gDefaultISpellLanguage
      else
        idx := -1;

    if idx > -1 then
      cbDefaultISpellLanguage.ItemIndex := idx;
  end
end;
...
begin
  ...
  if rbTextPartNNTP.Checked then
    ...
    if rbTextPartMIME.Checked then
      settings.TextPartStyle := tpMIME
    else
      if rbTextPartQuotedPrintable.Checked then
  ...
  if fISpellInstalled then
    settings.DefaultSpellLanguage := cbDefaultISpellLanguage.ItemIndex
  else
...
procedure TfmPropertyPagePosting.edMaxPostLineLengthChange(
  Sender: TObject);
begin
  UpdateData
end;
