...
uses Windows, Classes, SysUtils, ComObj, ActiveX, URLMon, unitMessages;
...
function TCIDMimeFilter.Abort(hrReason: HResult;
  dwOptions: DWORD): HResult;
begin
  result := S_OK;
end;

function TCIDMimeFilter.Continue(
...
function TCIDMimeFilter.Read(pv: Pointer; cb: ULONG;
  out cbRead: ULONG): HResult;
var
  data : PChar;
begin
  data := PChar (fData.Memory);
